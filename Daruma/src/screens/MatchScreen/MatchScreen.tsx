...
import { getFirestore, collection, getDocs, doc, getDoc, updateDoc, arrayUnion, arrayRemove, setDoc, addDoc, orderBy,limit, startAfter, QueryDocumentSnapshot } from 'firebase/firestore';
...
const MatchScreen = () => {
  ...
  const fetchProfiles = async () => {
    ...
    // A consulta inicial será com o limite de 50 perfis
    let q = query(collection(db, 'users'), orderBy('createdAt'), limit(50));
    if (lastVisibleProfile) {
      q = query(q, startAfter(lastVisibleProfile)); // Continua de onde parou na paginação
    }
  
    const querySnapshot = await getDocs(q);
  
    // Se a consulta não retornar perfis, significa que a base de dados foi completamente carregada
    if (querySnapshot.empty) {
      if (!allProfilesLoaded) {
        setAllProfilesLoaded(true); // Marca que todos os perfis foram carregados
      }
      setLastVisibleProfile(null); // Reseta o cursor de paginação
      return;
    }
  
    const newLastVisible = querySnapshot.docs[querySnapshot.docs.length - 1];
    setLastVisibleProfile(newLastVisible);
    ...
    matchSnapshot.forEach((doc) => {
      ...
      matchData.users?.forEach((uid: string) => {
        if (uid !== currentUser.uid) matchedUserIds.add(uid);
      });
    });
  
    const availableProfiles = filteredProfiles.filter(user =>
      !currentUserLikes.includes(user.id) && !matchedUserIds.has(user.id)
    );
  
    setProfiles(availableProfiles);
    setProfile(availableProfiles[Math.floor(Math.random() * availableProfiles.length)] || null);
    setLoading(false);
  };
  
  useEffect(() => {
    checkIfUserIsBanned(navigation);
  }, []);
  ...
  const getNextProfile = () => {
    if (profiles.length > 1 && profile) {
      ...
      if (newProfiles.length > 0) {
        ...
        // Se a lista de perfis estiver vazia, verifica se todos os perfis foram carregados
        if (allProfilesLoaded) {
          // Se todos os perfis foram carregados, reinicia a lista
          setAllProfilesLoaded(false); // Reseta a flag de "todos carregados"
          fetchProfiles(); // Carrega novamente desde o início
        } else {
          fetchProfiles(); // Caso contrário, carrega mais perfis
        }
      }
    } else {
      fetchProfiles(); // Se só houver um ou nenhum perfil, carrega mais perfis
    }
  };
